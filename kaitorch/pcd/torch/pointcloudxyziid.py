...
from ...attribute import ID, Intensity, XYZ


class PointCloudXYZIID(XYZ, Intensity, ID):
    r'''
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        super().__init__(
            xyzf=xyzf, intensity=intensity, id=id, *args, **kwargs
        )

    def __getitem__(self, i: Union[int, slice, Sequence[Union[int, bool]]]):
        return self.__class__(
            xyzf=self.xyz_[i], intensity=self.intensity_[i], id=self.id_[i]
        )
    ...
    def copy(self):
        ...
        '''
        return self.__class__(
            xyzf=self.xyz, intensity=self.intensity, id=self.id
        )
    ...
    def from_similar(cls, obj):
        ...
        '''
        return cls(xyzf=obj.xyz_, intensity=obj.intensity_, id=obj.id_)
