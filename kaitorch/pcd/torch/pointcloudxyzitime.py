...
from ...attribute import Intensity, Timestamp, XYZ


class PointCloudXYZITime(XYZ, Intensity, Timestamp):
    r'''
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        super().__init__(
            xyzf=xyzf, intensity=intensity, timestamp=timestamp,
            *args, **kwargs
        )
    ...
    def __getitem__(self, i: Union[int, slice, Sequence[Union[int, bool]]]):
        return self.__class__(
            xyzf=self.xyz_[i],
            intensity=self.intensity_[i],
            timestamp=self.timestamp_[i]
        )
    ...
    def copy(self):
        ...
        '''
        return self.__class__(
            xyzf=self.xyz, intensity=self.intensity, timestamp=self.timestamp
        )
    ...
    def from_similar(cls, obj):
        ...
        '''
        return cls(
            xyzf=obj.xyz_, intensity=obj.intensity_, timestamp=obj.timestamp_
        )
