...
from ...attribute import Intensity, Ring, XYZ


class PointCloudXYZIR(XYZ, Intensity, Ring):
    r'''
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        super().__init__(
            xyzf=xyzf, intensity=intensity, ring=ring, *args, **kwargs
        )

    def __getitem__(self, i: Union[int, slice, Sequence[Union[int, bool]]]):
        return self.__class__(
            xyzf=self.xyz_[i], intensity=self.intensity_[i], ring=self.ring_[i]
        )
    ...
    def copy(self):
        ...
        '''
        return self.__class__(
            xyzf=self.xyz, intensity=self.intensity, ring=self.ring
        )
    ...
    def from_similar(cls, obj):
        ...
        '''
        return cls(xyzf=obj.xyz_, intensity=obj.intensity_, ring=obj.ring_)
