...
from ...attribute import Intensity, XYZ


class PointCloudXYZI(XYZ, Intensity):
    r'''
    ...
    - intensity: intensities. Its shape should be `(N >= 0, 1)` or `(N >= 0,)`.
    ...
    #### Static Methods:
    - format_intensity: Make sure the shape of `intensity` is `(N >= 0, 1)`.
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        super().__init__(xyzf=xyzf, intensity=intensity, *args, **kwargs)

    def __getitem__(self, i: Union[int, slice, Sequence[Union[int, bool]]]):
        return self.__class__(xyzf=self.xyz_[i], intensity=self.intensity_[i])
    ...
    def append_(
        ...
        *args, **kwargs
        ...
        '''
        return super().append_(xyzf=xyzf, intensity=intensity, *args, **kwargs)
    ...
    def copy(self):
        ...
        '''
        return self.__class__(xyzf=self.xyz, intensity=self.intensity)
    ...
    def from_similar(cls, obj):
        ...
        '''
        return cls(xyzf=obj.xyz_, intensity=obj.intensity_)
