...
def distance_from_xyz(
    ...
    ### Args:
        - xyzf: coordinates in the 3D rectangular coordinate system. Its shape
            should be `([*,] 3 [+ C])`. A data should be in the form of
            `(x, y [, features])`.
...
def phi_from_xyz(xyzf: TorchTensor[TorchFloat]) -> TorchTensor[TorchFloat]:
    ...
    ### Args:
        - xyzf: coordinates in a 3D rectangular coordinate system. Its shape
            should be `([*,] 3 [+ C])`. A data should be in the form of
            `(x, y, z [, features])`.
...
def phi_from_rtz(rtzf: TorchTensor[TorchReal]) -> TorchTensor[TorchFloat]:
    ...
    ### Args:
        - rtzf: coordinates in a cylindrical coordinate system. Its shape
            should be `([*,] 3 [+ C])`.A data should be in the form of
...
def rho_from_xy(xyf: TorchTensor[TorchFloat]) -> TorchTensor[TorchFloat]:
    ...
    ### Args:
        - xyf: coordinates in the rectangular coordinate system. Its shape
            should be `([*,] 2 [+ C])`. A data should be in the form of
            `(x, y [, features])`.
...
def xy_to_rt(xyf: TorchTensor[TorchFloat]) -> TorchTensor[TorchFloat]:
    ...
    ### Args:
        - xyf: coordinates in a rectangular coordinate system. Its shape should
            be `([*,] 2 [+ C])`. A data should be in the form of
            `(x, y [, features])`.

    ### Returns:
        - Coordinates in a polar coordiante system. Its shape is `([*,] 2)`.
            The coordinates of a data is in the form of `(rho, theta)`. `rho`
            and `theta` are calculated as follows.
                    rho = sqrt(x ^ 2 + y ^ 2)
                    theta = arctan(y, x)
            `theta` is in radius.
...
def xyz_to_dtp(xyzf: TorchTensor[TorchFloat]) -> TorchTensor[TorchFloat]:
    ...
    ### Returns:
        - Coordinates in a spherical coordinate system. Its shape is
            `([*,] 3)`. The coordinates of a data is in the form of
            `(distance, theta, phi)`. `rho`, `theta` and `phi` are calculated
            as follows.
...
def xyz_to_rtz(xyzf: TorchTensor[TorchFloat]) -> TorchTensor[TorchFloat]:
    ...
    ### Returns:
        - Coordinates in a cylindrical coordinate system.
            Its shape is `([*,] 3)`. The coordinates of a data is in the form
            of `(rho, theta, z)`. `rho` and `theta` are calculated as follows.
                    rho = sqrt(x ^ 2 + y ^ 2)
                    theta = arctan(y, x)
            `theta` is in radius.
...
def rt_to_xy(rtf: TorchTensor[TorchReal]) -> TorchTensor[TorchFloat]:
    ...
    ### Args:
        - rtf: coordinates in a polar coordinate system. Its shape should be
            `([*,] 2 [+ C])`. A data should be in the form of
            `(rho, theta [, features])`.

    ### Returns:
        - Coordinates in a rectangular coordinate system. Its shape is
            `([*,] 2)`. The coordinates of a data is in the form of
            `(x, y)`. `x` and `y` are calculated as follows.
                    x = cos(theta) * rho
                    y = sin(theta) * rho
...
def rtz_to_xyz(rtzf: TorchTensor[TorchReal]) -> TorchTensor[TorchFloat]:
    ...
    ### Returns:
        - Coordinates in a 3D rectangular coordinate system. Its shape is
            `([*,] 3)`. The coordinates of a data is in the form of
            `(x, y, z)`. `x`, `y` and `z` are calculated as follows.
                    x = cos(theta) * rho
                    y = sin(theta) * rho
                    z = z
...
def dtp_to_rtz(dtpf: TorchTensor[TorchReal]) -> TorchTensor[TorchFloat]:
    ...
    ### Args:
        - dtpf: coordinates in a spherical coordinate system. Its shape should
            be `([*,] 3 [+ C])`.A data should be in the form of
            `(distance, theta_s, phi [, features])`.

    ### Returns:
        - Coordinates in a cylindrical coordinate system. Its shape is
            `([*,] 3)`. The coordinates of a data is in the form of
            `(rho, theta_c, z)`. `rho`, `theta_c` and `z` are calculated as
            follows.
                    rho = sin(phi) * distance
                    theta_c = theta_s
                    z = cos(phi) * distance
            `theta_c` is in radius.
    '''
    distances = dtpf[..., 0]
    phis = dtpf[..., 2]
    return torch.stack(
